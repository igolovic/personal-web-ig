...
<?php
		...
		{	
			...
			{
				case "delete":	
					if($conn->mysqli->query("delete from image_text_modules where image_text_module_id = '".$_POST["inpId"]."' and part_id = 'FavLinks'") == true)
					{
						$conn->mysqli->query("update image_text_modules set _order = (_order - 1) where _order > '".$_POST["inpId2"]."' and part_id = 'FavLinks'");
						unlink("../" . $_POST["inpValue"]);
					}
				break;
				case "up":
					$conn->mysqli->query("update image_text_modules set _order = (_order + 1) where _order = '".($_POST["inpValue"]-1)."' and part_id = 'FavLinks'");
					$conn->mysqli->query("update image_text_modules set _order = (_order - 1) where image_text_module_id = '".$_POST["inpId"]."'");
				break;
				case "down":
					$conn->mysqli->query("update image_text_modules set _order = (_order - 1) where _order = '".($_POST["inpValue"]+1)."' and part_id = 'FavLinks'");
					$conn->mysqli->query("update image_text_modules set _order = (_order + 1) where image_text_module_id = '".$_POST["inpId"]."' and part_id = 'FavLinks'");
				break;
				case "save":
					$id = $_POST["inpId"];
					...
					$conn->mysqli->query("update image_text_modules set title='".$title."', text = '".$text."' " . $path_sql_chunk . " where image_text_module_id = '".$id."' and part_id = 'FavLinks'");
				...
				case "new":
					...
					$conn->mysqli->query("insert into image_text_modules(image_text_module_id, part_id, _order, title, text, image_path, is_image) values(null, 'FavLinks', ".$conn->mysqli->query("select ifnull(max(_order)+1, 1) as num from image_text_modules where part_id = 'FavLinks'")->fetch_array()["num"].", '".$title."', '".$text."', '" .$path. "', 1)");
				...
				break;
				case "upL":
				//echo "update image_text_submodules set _order = (_order + 1) where _order = '".($_POST["inpValue"]-1)."' and module = '".$_POST["inpId2"]."' and position = '0'";
				//echo "<br/>";
				//echo "update image_text_submodules set _order = (_order - 1) where image_text_submodule_id = '".$_POST["inpId"]."'";
				...
				case "newL":
					...
					echo "insert into image_text_submodules(image_text_submodule_id, _order, part_id, position, module, title, text) values(null, ".$conn->mysqli->query("select ifnull(max(_order)+1, 1) as num from image_text_submodules where part_id = 'FavLinks' and position = 0")->fetch_array()["num"].", 'FavLinks', 0, ".$_POST["inpId"].", '".$title."', '".$text."')";
					$conn->mysqli->query("insert into image_text_submodules(image_text_submodule_id, _order, part_id, position, module, title, text) values(null, ".$conn->mysqli->query("select ifnull(max(_order)+1, 1) as num from image_text_submodules where part_id = 'FavLinks' and position = 0")->fetch_array()["num"].", 'FavLinks', 0, ".$_POST["inpId"].", '".$title."', '".$text."')");
				break;
				...
				case "newR":
					...
					$conn->mysqli->query("insert into image_text_submodules (image_text_submodule_id, _order, part_id, position, module, title, text) values(null, ".$conn->mysqli->query("select ifnull(max(_order)+1, 1) as num from image_text_submodules where part_id = 'FavLinks' and position = 1")->fetch_array()["num"].", 'FavLinks', 1, ".$_POST["inpId"].", '".$title."', '".$text."')");
				break;			}
		}
				
		// get favorite programs
		$favprogsResult = $conn->mysqli->query("select * from image_text_modules where part_id = 'FavLinks' order by _order");
		unset($conn);
		$counter = 0;
		$rownum = $favprogsResult->num_rows;

		echo "<form enctype='multipart/form-data' method='POST' id='formCv'> <input type='hidden' name='inpAction' id='inpAction'>";
		...
		while($row = $favprogsResult->fetch_array())
		{
			...
			echo '<button onclick="handleClick(\'delete\',\''.$row["image_text_module_id"].'\',\''.$row["_order"].'\',\''.htmlspecialchars($row["image_path"]).'\')">obriši</button><br/>';
			if($counter > 1)
			{
			...
			echo '<button onclick="handleClick(\'save\',\''.$row["image_text_module_id"].'\',\''.$row["_order"].'\',\''.htmlspecialchars($row["image_path"]).'\')">spremi</button></td>';
